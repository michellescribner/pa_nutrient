...
nrow(snps_noref)#138
write.csv(ancestor_snps, file="ancestor_snps.csv")
...
snps_final <- separate(snps_noref, col = Population, sep = ", ", into = c("b_p", "pop", "day"), remove=FALSE)
...
nje <- as_tibble(nje)
njemap <- nje %>% select(Population, num, position, freq, annotation, gene, product)
...
c <- snps_final %>%
  ...
  theme_minimal()+scale_x_continuous(limits = c(0,6537648), breaks = c(2e6, 4e6, 6e6),label = c("2,000,000", "4,000,000", "6,000,000"), expand = c(0,0))+
...
genes %>% 
  ...
  scale_x_continuous(limits = c(4070000, 4140000), breaks = c(4.08e6, 4.1e6, 4.12e6, 4.14e6),label = c("4,080,000", "4,100,000", "4,120,000", "4,140,000"),position = "top")+
  scale_y_continuous(limits = c(0, 7))+
  theme_minimal() +  xlab("Genome Position (bp)") +
  theme(legend.position = "none", axis.line = element_blank(), axis.text.y = element_blank(), axis.title.y = element_blank(),
...
snps_final %>% 
  ...
  theme(legend.position = "none", panel.grid.minor = element_blank(), axis.text = element_text(size=12, color = "black"), axis.title = element_text(size=12))+
...
morA%>% 
  ...
  theme_minimal() + theme(panel.grid.minor = element_blank(), axis.text = element_text(size = 12, color = "black"))+ 
